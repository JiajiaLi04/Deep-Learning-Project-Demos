...
def get_rgb_cat(xs, colors):
    ...
    for i in range(0, np.shape(xs)[0], batch_size):
        ...
        nexts.append(next)
    result = np.concatenate(nexts, axis=0)  # make the result
    return result


# colors.shape(24,3)
# next.shape(100, 1, 32, 32)
# result.shape(50000, 1, 32, 32)
# nexts <class 'list'> _len_ 50. each ndarray's shape (100, 1, 32, 32)
...
class CNN(nn.Module):
    def __init__(self, kernel, num_filters, num_colors):
        ...
        self.upconv2 = nn.Sequential(
            MyConv2d(32, num_colors, kernel_size=kernel, padding=padding),
            nn.Upsample(scale_factor=2),
            nn.BatchNorm2d(num_colors),
            nn.ReLU())

        self.finalconv = MyConv2d(num_colors, num_colors, kernel_size=kernel, padding=padding)
        ###################################################
...
class UNet(nn.Module):
    def __init__(self, kernel, num_filters, num_colors):
        ...
        self.upconv1 = nn.Sequential(
            MyConv2d(num_filters * 2 + num_filters * 2, num_filters, kernel_size=kernel, padding=padding),
            nn.Upsample(scale_factor=2),
            nn.BatchNorm2d(num_filters),
            nn.ReLU())

        self.upconv2 = nn.Sequential(
            MyConv2d(num_filters * 2, num_colors, kernel_size=kernel, padding=padding),
            nn.Upsample(scale_factor=2),
            nn.BatchNorm2d(num_colors),
            nn.ReLU())

        self.finalconv = MyConv2d(num_colors + 1, num_colors, kernel_size=kernel, padding=padding)
    ...
    def forward(self, x):
        ...
        # self.out2 = self.downconv2(self.out1)
        # self.out3 = self.rfconv(self.out2)
        # self.out4 = self.upconv1(self.out3 + self.out2)
        # self.out5 = self.upconv2(self.out4 + self.out1)
        # self.out_final = self.finalconv(self.out5 + x)
        # return self.out_final
        ###################################################
        # pass
...
def compute_loss(criterion, outputs, labels, batch_size, num_colors):
    ...
    loss_out = outputs.transpose(1, 3).contiguous().view([batch_size * 32 * 32, num_colors])
...
if __name__ == '__main__':
    ...
    colors = np.load(args.colors, encoding='latin1')[0]
    num_colors = np.shape(colors)[0]
    # colors.shape（24,3) the value is normalization [0,1]
    # #####################################
    # # The second question 2.2
    # args.model = "CNN"
    # args.checkpoint = "weights/cnn_k3_f32.pkl"
    # args.valid = True
    # ###########################

    #############################
    # The third question 3.2
    args.model = "UNet"
    args.checkpoint = "weights/unet_k3_f32.pkl"
    args.batch_size = 10
    args.epochs = 25
    #############################
    # LOAD THE MODEL
