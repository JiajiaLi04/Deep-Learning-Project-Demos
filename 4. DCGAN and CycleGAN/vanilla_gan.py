...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = 1 / (2 * real_images.size(0)) * torch.sum((D(real_images) - 1).pow(2))
            ...
            D_fake_loss = 1 / (2 * real_images.size(0)) * torch.sum(D(fake_images).pow(2))
            ...
            G_loss = 1 / real_images.size(0) * torch.sum((D(fake_images)-1).pow(2))
