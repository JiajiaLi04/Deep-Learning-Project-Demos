...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters + 1):
        ...
        D_X_loss = 1 / images_X.size(0) * torch.sum((D_X(images_X) - 1).pow(2))
        D_Y_loss = 1 / images_Y.size(0) * torch.sum((D_Y(images_Y) - 1).pow(2))
        ...
        D_X_loss = 1 / images_Y.size(0) * torch.sum((D_X(fake_X)).pow(2))
        ...
        D_Y_loss = 1 / images_X.size(0) * torch.sum((D_Y(fake_Y)).pow(2))
        ...
        g_loss = 1 / images_Y.size(0) * torch.sum((D_X(fake_X) - 1).pow(2))
        ...
        if opts.use_cycle_consistency_loss:
            ...
            cycle_consistency_loss = 1 / images_Y.size(0) * torch.sum((images_Y - reconstructed_Y).pow(2))
        ...
        g_loss = 1 / images_X.size(0) * torch.sum((D_Y(fake_Y) - 1).pow(2))


        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = 1 / images_X.size(0) * torch.sum((images_X - reconstructed_X).pow(2))
