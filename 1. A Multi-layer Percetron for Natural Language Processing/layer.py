...
class WordEmbedding:
    ...
    def backward(self, grad_output):
        ...
        for i in range(self.input.size(1)):
            self.get_indicator_matrix(self.input[:, i].long())
            self.grad_weight += torch.mm(torch.t(self.indicator_matrix),
...
class Linear:
    ...
    def backward(self, grad_output):
        ...
        self.grad_weight = torch.mm(torch.t(self.input), grad_output)
        self.grad_bias = torch.sum(grad_output, 0)
        grad_linear = torch.mm(grad_output, torch.t(self.weight))
        return grad_linear
        #####################
...
class Sigmoid():
    ...
    def backward(self, grad_output):
        ...
        # pass
        # YOUR CODE HERE
        grad_sigmoid = torch.mul(grad_output, torch.mul(self.input, (1 - self.input)))
        return grad_sigmoid
        #####################
...
class SoftMax():
    ...
    def backward(self, grad_output):
        ...
        # pass
        # YOUR CODE HERE
        _, get_indicator_matrix = grad_output
        grad_softmax = self.prob - get_indicator_matrix
        return grad_softmax
        #####################
...
class CrossEntropy():
    ...
    def backward(self, grad_output):  # grad_output is loss
        ########################
        # pass
        # YOUR CODE HERE
        indicator_matrix = self.get_indicator_matrix(self.y)
        grad_input = torch.mul((-1 / (self.prob + EPS)), indicator_matrix)
        return grad_input, indicator_matrix
        ########################
